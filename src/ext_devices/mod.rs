...
impl ExtDevices {
    pub fn new(config: ExtDevicesConfig, sdl: &Rc<RefCell<Sdl>>) -> Result<Self> {
        ...
        let displays = config.display.unwrap_or_default().into_iter()
            .map(|config| Display::new(config, sdl).map(RefCell::new).map(Rc::new))
            .collect::<Result<_>>()?;

        let lcds = config.lcd.unwrap_or_default().into_iter()
            .map(|config| Lcd::new(config, sdl).map(RefCell::new).map(Rc::new))
            .collect::<Result<_>>()?;
...
use crate::{system::System, sdl::Sdl};
